...
from .model_registry import MODELS, get_cls
from .model_loading import ContrastiveDecodeWithDifferentPromptModel, load_tokenizer
import random
import os

from transformers import BatchEncoding, set_seed
from .model_loading import load_hf_model, make_model_cast_to_fp32
from .llama_contrastive_skip_model import get_dola_hook, get_skip_cd_cls, mask_layer
from .compute_skip_layer import compute_skip_layer
...
def setup_model(
    ...
    *,
    ...
    if algorithm == 'direct':
        ...
        model = load_hf_model(model_dir, bit=quant, autoload_cls=get_skip_cd_cls(model_cls))
        make_model_cast_to_fp32(model)
        ...
        print(f'selected based on heuristic: [{start}, {end})')
